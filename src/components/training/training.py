...
class ModelTrainer:
    ...
    def evaluate_features(self, target_feature_name="EvaluateAllNumericFeatures"):
        ...
        if target_feature_name == "EvaluateAllNumericFeatures":
            ...
            categorical_features, numeric_features = (
                data_transformer_obj.split_features(df_raw)
            )
            # Iterate through all numerical features and perform model evalulation for each
            for target_feature in numeric_features:
                train_arr, test_arr, pre_proc_obj_path = (
                    data_transformer_obj.initiate_data_transformation(target_feature)
                )
                model_report = self.initiate_model_trainer(
                    train_arr, test_arr, target_feature
                )
        else:
            # Load Raw data and extract all numerical features
            train_arr, test_arr, pre_proc_obj_path = (
                data_transformer_obj.initiate_data_transformation(target_feature_name)
            )
            model_report = self.initiate_model_trainer(
                train_arr, test_arr, target_feature_name
            )
    ...
    def evaluate_models(self, X_train, y_train, X_test, y_test, models):
        try:
            report = {}
            hyperparameter_tuning_specs = (
                self.model_trainer_config.hyperparameter_tuning_specs
            )
            ...
            for name, model in models.items():
                ...
                # logging.info(f"Model {name} has a test R2 score of {train_model_score}")
                results = dict(
                    sorted(report.items(), key=lambda item: item[1], reverse=True)
                )
            return results
        ...
        try:
            ...
            if best_model_score < 0.6:
                raise CustomException(
                    "No models passing minimum criteria of R2 score > 0.6."
                )
            logging.info(
                f"The best model found was {best_model_name} with a R2 score of {best_model_score}"
            )
